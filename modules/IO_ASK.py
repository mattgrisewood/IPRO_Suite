...
def for_usage_limitations(experiment, default):
    ...
    if usagePatterns[-1] != "/":
        usagePatterns += "/"
    # Store the value
    experiment["OptCDR Usage Patterns"] = experiment["OptCDR Path"] + \
                                          usagePatterns
...
def for_antigen_rotations(experiment, default):
    ...
    while not accept:
        rotations = {}
        # Go through all of the available molecules
        for mn in experiment["Molecules"]:
            # Skip any design molecules, though they should not exist when this
            # function is called
            if mn[2].design:
                continue
            # If the user is just using the defaults
            if default:
                rotations[mn[2].name] = standardAntigenRotation
            # Otherwise, for all antigens (Target Molecules), ask for antigen 
            # rotation permissions
            else:
                question = "Would you like to 'maintain' the current " + \
                "orientation of " + mn[3] + " " + mn[2].name + ", or allow " + \
                "it to 'rotate' freely?"
                answer = FUNCTIONS.answer_question(question, "str", [], \
                                                   ['maintain', 'rotate'])
                # Store the user's response in the 'rotations' dictionary
                rotations[mn[2].name] = answer
        # Validate the provided inputs with the user
        if not default:
            summary = '\nYou have specified the following antigen rotation ' + \
            'permissions:\n'
            for mn in experiment["Molecules"]:
                if not mn[2].design:
                    summary += mn[2].name + ' - ' + \
                               rotations[mn[2].name].upper() + ' orientation\n'
            summary += '\nAre you sure the inputs specified above are correct?'
            accept = FUNCTIONS.answer_question(summary, "bool")
            os.system("clear")
        else:
            accept = True
    experiment["OptCDR Rotations"] = rotations 
