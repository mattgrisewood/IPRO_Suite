...
def OptCDR_info(experiment):
    """Check the values of the attributes for how to run OptCDR."""
    # Store the errors here
    errors = ''
    # Go through the OptCDR attributes
    for name in ['Optcdr Path', 'Optcdr Canonical Path', \
                 'Optcdr Clash Path', 'Optcdr Gaussian Path', \
                 'Optcdr Cluster Folder', 'Optcdr Rotations', 'Optcdr Chains', \
                 'Optcdr Frameworks', 'Optcdr Positions', 'Optcdr Libraries', \
        ...
        # Otherwise, try to check it
            ...
            try:
                CHECK.OptCDR_info(name, experiment[name], experiment)
            except FUNCTIONS.IPRO_IOError as error:
                errors += str(error)
    return errors                
