...
def translate_attribute_name(name, computer = True):
    ...
    for entry in spacingWords:
        ...
        for i in range(1, len(entry)):
            ...
            # start a new word. Skip Opt since this is the convention within the
            # Maranas Lab
            if entry[i].isupper() and not entry[i-1].isupper() and \
            word != "Opt":
                words.append(word)
                word = ''
...
class Experiment(object):
    ...
    def __setattr__(self, name, value):
        ...
        'Docking End Temp']:
            ...
            object.__setattr__(self, internal, value)
        # If the attribute relates to how to run OptCDR
        elif external in ['Optcdr Path', 'Optcdr Canonical Path', \
        'Optcdr Clash Path', 'Optcdr Gaussian Path', 'Optcdr Cluster Folder', \
        'Optcdr Rotations', 'Optcdr Chains',  'Optcdr Frameworks', \
        'Optcdr Positions', 'Optcdr Libraries', 'Optcdr Usage Patterns']:
            IO_CHECK.OptCDR_info(external, value, self)
            object.__setattr__(self, internal, value)
    ...
    def validate(self):
        ...
        errors = ''
        # Force field
        if self["Type"] in ['IPRO', 'Mutator', 'OptCDR', 'OptMAVEn', 'OptZyme']:
        ...
        # How to run OptCDR
        if self["Type"] in ['OptCDR']:
            errors += IO_VALIDATE.OptCDR_info(self)
            errors += IO_VALIDATE.EpitopePositions(self)
        ...
        # Design Groups
        if self["Type"] in ["IPRO", "Mutator", 'OptCDR', 'OptMAVEn']:
        ...
        if self["Type"] in ["IPRO", "Mutator", "OptCDR", "OptZyme"]:
            errors += IO_VALIDATE.Restraints(self)
        if errors != '':
            raise ExperimentError(errors[1:])

    def miracle_load(self, load):
        ...
        if self["Type"] in ["OptCDR"]:
            errors += IO_LOADING.OptCDR_info(self, data)
            errors += IO_LOADING.EpitopePositions(self, data)
        if self["Type"] in ["IPRO", "Mutator", "OptCDR", "OptZyme"]:
    ...
    def output(self):
        ...
        if self["Type"] in ["OptCDR"]:
            text += IO_OUTPUT.OptCDR_info(self)
        if self["Type"] in ["IPRO", "Mutator", "OptCDR", "OptMAVEn", "OptZyme"]:
    ...
    def user_creation(self):
        ...
        if self["Type"] in ["Mutator"]:
            ...
            IO_ASK.Mutants(self)
        if self["Type"] in ["OptCDR"]:
            IO_ASK.OptCDR_info(self) 
        if self["Type"] in ["IPRO", "OptCDR", "OptMAVEn", "OptZyme"]:
