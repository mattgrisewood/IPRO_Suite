...
def OptCDR_info(experiment, data):
    """Load the OptCDR information"""
    # Store the errors here
    errors = ''    
    # Some of the OptCDR experiment attributes were formatted to make them user
    # readable. Change them back to the formatting compatible with the computer
    for user, computer in zip(['Home Directory', 'Canonical Folder',  
                               'Clash File', 'Position File',  
                               'Cluster Folder'], ['Optcdr Path',  
                               'Optcdr Canonical Path', 'Optcdr Clash Path',   
                               'Optcdr Gaussian Path','Optcdr Cluster Folder']):
        data[computer] = data[user]                           
    # Load the OptCDR home directory
    errors += standard_load('Optcdr Path', experiment, data, 'string')
    # Load the folders and files within the OptCDR home directory
    for name in ['Optcdr Canonical Path', 'Optcdr Clash Path',  
                 'Optcdr Gaussian Path', 'Optcdr Cluster Folder']:
        # Add the OptCDR home directory to the file/folder path
        data[name] = experiment['Optcdr Path'] + data[name]
        errors += standard_load(name, experiment, data, 'string')
    return errors
