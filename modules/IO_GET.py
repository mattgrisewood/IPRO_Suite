...
def EpitopePositions(positions, molecule, experiment):
    ...
    while not accept:
        ...
        # If a selection was made, confirm it is right
        if pick:
            # If the selected residues will lead to an epitope with more than 15
            # residues, display a warning to the user
            if (len(chosen) + len(residues)) >= 15:
                question = "WARNING: The number of residues selected is 15 or " 
                question += "higher. This is a large epitope and may not work "
                question += "properly due to its large size\n"
            else:
                question = ""
            # Get the question differently depending on how many Residues were
            # picked
            if len(residues) == 1:
                question += "Are you certain Residue " + residues[0] + " should"
                question += " be used as a Epitope Position?"
            else:
                question += "Are you certain the " + str(len(residues))
                question += " Residues between and inclusive of Residues "
                question += residues[0] + " and " + residues[-1] + " should be "
